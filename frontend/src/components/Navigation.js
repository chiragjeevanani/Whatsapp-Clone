...
export default function Navigation({ activeTab }) {
  ...
  return (
    <nav className="fixed bottom-0 w-full z-50 bg-white border-t border-zinc-100 pb-safe shadow-[0_-2px_10px_rgba(0,0,0,0.03)]">
      <div className="flex justify-around items-center h-[76px] w-full px-1 max-w-3xl mx-auto relative overflow-hidden">
        ...
        {activeIndex !== -1 && (
          <motion.div
            ...
            className="absolute h-8 bg-[#e6f5ef] rounded-full z-0 pointer-events-none"
            style={{
              width: "64px",
            ...
            >
              ...
              <div className="relative">
                <motion.div
                  ...
                  className={`flex items-center justify-center rounded-full px-6 py-1 transition-colors duration-200 ${
                    isActive ? "text-[#0f8b5d]" : "text-[#54656f] hover:bg-zinc-50/50"
                  }`}
                >
                  <span
                ...
                {/* Badge Overlay */}
                {tab.badge && (
                  <span className="absolute -top-1.5 -right-2.5 bg-[#00a884] text-white text-[9.5px] font-bold px-1.5 py-0.5 rounded-full border-2 border-white min-w-[18px] text-center leading-none">
                    {tab.badge}
                  </span>
                )}
                ...
                {/* Green Dot Indicator */}
                {tab.hasDot && (
                  <span className="absolute top-0 right-1 w-2.5 h-2.5 bg-[#00a884] rounded-full border-2 border-white"></span>
                )}
              </div>
              ...
              <span
                className={`text-[12px] mt-1.5 transition-colors tracking-wide ${
                  isActive ? "text-[#1c2e35] font-bold" : "text-[#54656f] font-medium"
                }`}
              >
                {tab.label}
              </span>
